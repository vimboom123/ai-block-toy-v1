...
| `give_hint` | 给最轻量提示，不直接给答案 | `assistant.reply_prepared`, `tts.playback_finished`, `parent.interrupt_requested` | `help_level_current=none`，首次失败或低置信 | 写 `help.level_changed none->light_nudge`，增加 `retry_count` | 只给方向，不说完整答案，不拆很多步 | 提示播报完成回 `await_answer` |
