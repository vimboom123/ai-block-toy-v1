...
`06-session-runtime` 是当前项目最完整、最接近“产品主链路”的可运行模块。它在 [`../05-dialog-runtime/`](../05-dialog-runtime/) 的真实对话能力之上，增加了 session、turn、task 推进、最小持久化，以及同端口 UI + HTTP API。
...
- `session_runtime/phase5_bridge.py`
  - 复用 05-dialog-runtime 的 scene 和 Ark responder
...
```text
06-session-runtime/state/session-runtime-store.json
```
