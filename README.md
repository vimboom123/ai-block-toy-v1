# AI积木玩具 v1

这坨文档我已经单独收进这个文件夹，别再和 `studio/projects` 里别的项目混成一锅粥。

## 当前文档

1. `ai-block-toy-master-outline-v1.md`
   - 总纲 / 主线基线

2. `ai-block-toy-state-machine-v1.md`
   - A：状态机主流程

3. `ai-block-toy-backend-schema-v1.md`
   - B：后端事件 / 数据结构

4. `ai-block-toy-app-sync-map-v1.md`
   - C：状态机 / 事件流 / 家长端手机 App 同步映射

## 建议阅读顺序

1. 先看 `ai-block-toy-master-outline-v1.md`
2. 再看 `ai-block-toy-state-machine-v1.md`
3. 再看 `ai-block-toy-backend-schema-v1.md`
4. 最后看 `ai-block-toy-app-sync-map-v1.md`

## 当前状态

- 这套文档已经完成一轮收口和必改项修补。
- `public_stage / session.status / paused / aborted / ended` 的口径已经重新对齐。
- Projection / DTO 字段来源简表已经补进同步映射文档。

## 备注

- 之前提到的家长端 UI 主稿文件，目前在这个目录里没找到落盘成品；如果要，我下一步可以补一份正式归档版到这个文件夹里。
