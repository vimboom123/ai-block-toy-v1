...
`05-dialog-runtime` 是项目当前最小的“真实模型对话运行时”模块。它负责把 Fire Station 场景定义、prompt 构建和 Ark 文本调用接起来，输出结构化的 task-level guidance 结果。
...
- 提供一个超薄 HTTP bridge，方便 `ui-mvp-mobile/` 联调
- 作为 [`../06-session-runtime/README.md`](../06-session-runtime/README.md) 的底层 responder 来源
...
- `runtime/ui_runtime_server.py`
  - 提供最小 runtime API，并同时服务 `ui-mvp-mobile/`
...
如果需要给 `ui-mvp-mobile/` 提供真实 runtime 数据，可以直接启动本目录的本地 server：
...
如果你想看“当前用户可见的完整演示链路”，优先从 [`../06-session-runtime/README.md`](../06-session-runtime/README.md) 开始。
