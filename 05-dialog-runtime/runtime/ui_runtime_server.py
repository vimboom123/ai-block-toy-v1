...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Serve ui-mvp-mobile and expose a minimal Fire Station runtime JSON API."
    )
    parser.add_argument(
        "--host",
        default="127.0.0.1",
    ...
    parser.add_argument(
        ...
        help="Scene pack path relative to the 05-dialog-runtime folder.",
    )
    parser.add_argument(
        "--task-id",
        action="append",
...
def main() -> int:
    ...
    project_root_dir = runtime_root_dir.parent
    ui_dir = project_root_dir / "ui-mvp-mobile"

    if not ui_dir.is_dir():
        raise SystemExit(f"ui-mvp-mobile directory not found: {ui_dir}")
