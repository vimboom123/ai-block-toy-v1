...
| `parent_user_id` | `uuid` | 是 | 归属家长账号 ID，用于家长端读权限判定 | 否 |
...
| `requires_self_report` | `boolean` | 是 | 该任务完成后是否必须进入儿童自报确认链路 | 否 |
| `help_level_current` | `enum(help_level)` | 是 | 当前任务当前帮助级别 | 是 |
| `help_level_peak` | `enum(help_level)` | 是 | 当前任务内已达到过的最高帮助级别，供 report / 复盘投影使用 | 是 |
| `success_condition` | `jsonb` | 是 | 完成条件快照 | 否 |
| `result_code` | `enum(task_result_code)` | 否 | 完成/失败结果码 | 是 |
| `activated_at` | `timestamptz` | 否 | 被激活进入当前任务的时间，用于 live/timeline 投影排序和 fallback | 否 |
...
| `publish_status` | `enum(report_status)` | 是 | `draft / published / partial / withdrawn` | 否 |
...
| `state.transition_applied` | `state_engine` | `rule_id`, `from_state`, `to_state`, `trigger_event`, `task_id` | 真正推进状态机；若转移与某个 task 语义绑定，必须带 `task_id` 供 timeline/live 投影使用 | 否 |
