...
from .fire_station_session_planner import FireStationSessionPlanner
...
# Phase 6 / UI sessions should default to the full Fire Station chain.
        ...
        selected.append(
            {
                ...
                "parent_label": str(task.get("parent_label") or task.get("name") or ""),
                "requires_self_report": bool(task.get("requires_self_report")),
                "assistant_led_summary": bool(task.get("assistant_led_summary")),
            }
        )
...
class Phase5FireStationTurnResponder:
    ...
    def generate_reply(
        ...
        if resolved_task_signal == "task_completed" and upcoming_task is None:
            return AssistantTurnResult(
                prompt_version=TURN_PROMPT_VERSION,
                reply_text="说得真清楚，这次消防任务顺利完成啦。",
        ...
        try:
            ...
            system_prompt = "\n".join(
                [
                    ...
                    "reply_text 可以自然一点，不必刻意压得很短；只要别长篇大论。",
                    "next_expected_action 说清下一步要做什么就行，不必刻意压字数。",
            ...
            context_prompt = "\n".join(
                [
                    ...
                    f"task_goal: {current_task.goal or task.get('goal') or ''}",
...
def build_default_runtime_service(
    ...
    planner = FireStationSessionPlanner()
